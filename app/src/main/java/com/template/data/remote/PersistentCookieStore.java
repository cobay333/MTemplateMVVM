...
public class PersistentCookieStore {
    ...
    public void add(HttpUrl url, Cookie cookie) {
        ...
        ConcurrentHashMap<String, Cookie> conCurHashMap = cookies.get(url.host());
        Set<String> cookieSet;
        if (conCurHashMap == null || conCurHashMap.size() == 0) {
            cookieSet = new HashSet<>();
        } else {
            cookieSet = conCurHashMap.keySet();
        }
        prefsWriter.putString(url.host(), TextUtils.join(",", cookieSet));
